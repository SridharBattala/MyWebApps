...
public class CoinChangingMinimumCoin {
	 ...
	 */
	public int minimumCoinTopDown(final int total, final int coins[], final Map<Integer, Integer> map) {
		...
		for (int i = 0; i < coins.length; i++) {
			...
			final int val = minimumCoinTopDown(total - coins[i], coins, map);
	 ...
	 * possible.
	 */
	public int minimumCoinBottomUp(final int total, final int coins[]) {
		final int T[] = new int[total + 1];
		final int R[] = new int[total + 1];
		T[0] = 0;
	...
	private void printCoinCombination(final int R[], final int coins[]) {
		if (R[R.length - 1] == -1) {
	...
	public static void main(final String args[]) {
		final int total = 13;
		final int coins[] = { 7, 3, 2, 6 };
		final CoinChangingMinimumCoin cc = new CoinChangingMinimumCoin();
		final Map<Integer, Integer> map = new HashMap<>();
		final int topDownValue = cc.minimumCoinTopDown(total, coins, map);
		final int bottomUpValue = cc.minimumCoinBottomUp(total, coins);
